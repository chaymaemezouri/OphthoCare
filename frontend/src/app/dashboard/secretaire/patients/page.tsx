...
export default function SecretairePatientsPage() {
  ...
  React.useEffect(() => {
    if (authLoading || user?.role !== "secretary") return;
    let cancelled = false;
    (async () => {
      setLoading(true);
      setError(null);
  ...
  if (authLoading || user?.role !== "secretary") {
    return (
        ...
        <div className="flex items-end justify-between">
          <div className="space-y-1">
            ...
            <p className="text-[13px] text-slate-500 font-medium">
              Patients de l&apos;espace cabinet (après un premier RDV confirmé) — lecture du dossier
            </p>
          </div>
          <Button
            variant="outline"
                        ...
                        <TableCell className="text-right px-6">
                          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button
                              ...
                              asChild
                            >
                              <Link href={`/dashboard/secretaire/patients/${patient.id}`}>
